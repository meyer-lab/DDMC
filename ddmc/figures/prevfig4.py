...
def makeFigure():
    ...
    plot_roc(ax[2], lr, model.transform(), y, cv_folds=4, return_mAUC=False)
    ax[2].set_title("DDMC ROC")

    # Tumor vs NAT unclustered
    plot_roc(ax[0], lr, d.values, y, cv_folds=4, title="ROC unclustered")
    ...
    plot_roc(ax[3], lr, kmeans.cluster_centers_.T, y, cv_folds=4, title="ROC k-means")
    ...
    plot_roc(ax[4], lr, gmm.transform(), y, cv_folds=4, title="ROC GMM")
