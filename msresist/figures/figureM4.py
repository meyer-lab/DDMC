...
def makeFigure():
    ...
    ax, f = getSetup((20, 12), (3, 6), multz={3:1, 9:1, 15:1})
    ...
    predict_mutants(ax[:3], model.transform(), y, lr1, "TP53.mutation.status")
    cc["TP53 status"] = yy["TP53.mutation.status"].values
    plot_clusters_binaryfeatures(cc, "TP53 status", ax[3])
    pvals = build_pval_matrix(calculate_mannW_pvals(cc, "TP53 status", 1, 0), model.ncl)
    sns.barplot(x="p-value", y="Clusters", data=pvals, hue="Significant", orient="h", ax=ax[4])
    cc = cc.drop("TP53 status", axis=1)
