...
def test_clusters(w, ncl, distance_method):
    ...
    MSC = DDMC(info, ncl, SeqWeight=w, distance_method=distance_method, pre_motifs=preMotifSet[0:ncl]).fit(X=data)
...
def test_pickle(distm):
    ...
    MSC = DDMC(info, 5, SeqWeight=2, distance_method=distm, pre_motifs=preMotifSet[0:2]).fit(X=data)
