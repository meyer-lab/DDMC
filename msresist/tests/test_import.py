...
class TestImport(unittest.TestCase):
    ...
    def test_import(self):
        ...
        ABC_mc1 = preprocessing(AXLwt=True, FCfilter=True, log2T=False)
        self.assertTrue(np.all(np.min(ABC_mc1.iloc[:, 5:])) > 0.0)

        ABC_mc2 = preprocessing(AXLwt=True, FCfilter=False, log2T=False)
        self.assertTrue(np.all(np.min(ABC_mc2.iloc[:, 5:])) > 0.0)

        # If we don't have log2-transformed values, none should be negative
        ABC_mc4 = preprocessing(AXLwt=True, Vfilter=True, FCfilter=True, log2T=False)
        self.assertTrue(np.all(np.min(ABC_mc4.iloc[:, 5:])) > 0.0)

        ABC_mc5 = preprocessing(AXLwt=True, Vfilter=True, FCfilter=False, log2T=False)
