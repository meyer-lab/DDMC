...
def calculatedistances_logmean(file, mutant, treatment, replicate, vs_count, cells=(1,3)):
...
def PlotRipleysK(folder, mutant, treatments, replicates, ax):
    ...
    b = sns.lineplot(x='Radii', y='K Estimate', hue='Condition', data=df, ci=68, ax=ax)
    ax.set_title(mutant)


def BarPlotRipleysK(folder, mutants, treatments, replicates, radius, ax):
    ...
    b = sns.barplot(x='Mutant', y='K Estimate', hue='Treatment', data=df, ci=68, ax=ax)
...
def PlotRipleysK_TimeCourse(folder, extensions, timepoint, ax):
    ...
    for idx in range(len(extensions)):
        file = pd.read_csv("msresist/data/Distances/"+folder+"/Results_"+extensions[idx]+'_'+str(timepoint)+".csv")
        points = file.loc[:, "X":"Y"].values
